...
const PATCH_STRUCTURE_FIELDS = {
  ...
  'inquiry-box': {
    ...
    inquiryTitleLabel: 'inquiryTitleLabel',
...
function main() {
  ...
  try {
    if (ir.mode === 'patch') {
      ...
      designJson = ir.mode === 'replace' || !canonicalEnvelope
        ? compiledDesignJson
...
function applyPatchOperations(currentDesignJson, operations) {
  ...
  const designJson = structuredClone(currentDesignJson);
  for (const [operationIndex, operation] of operations.entries()) {
    ...
    if (op === 'replace' || op === 'replace-section') {
      if (location.root === location.component) {
        ...
        } else if (operation.component && TOP_LEVEL_BLOCK_TYPES.has(normalizeType(operation.component.type))) {
          location.parent[location.index] = compileTopLevelBlock({
            ...operation.component,
...
function updateCanonicalComponent(component, changes, operationIndex) {
  ...
  for (const [key, value] of Object.entries(changes)) {
    ...
    if (type === 'img' && (key === 'src' || key === 'url')) {
      const upload = component?.field?.structure?.child?.upload;
      if (!upload || !Object.hasOwn(upload, 'value') || !upload.value || typeof upload.value !== 'object') {
        throw new Error(`Field ${key} is not available on component ${component.id}.`);
      }
      upload.value.url = string(value);
      continue;
    }
...
function setFieldControl(component, group, field, value, requestedField) {
  ...
  if (!target || !Object.hasOwn(target, 'value')) {
...
function validatePatchedLayout(component, root, changes, operationIndex) {
  ...
  const layoutKeys = ['x', 'left', 'y', 'top', 'w', 'width', 'h', 'height', 'text', 'content', 'html', 'fontSize', 'lineHeight'];
  if (!layoutKeys.some((key) => changedKeys.has(key)) || component.type === 'free-box') return;
...
function validateIr(input, { hasCanonical = false } = {}) {
  ...
  if (mode !== 'extend' && mode !== 'replace' && mode !== 'patch') {
    errors.push('mode must be "extend", "replace", or "patch"');
  }
  if (mode === 'patch') {
    if (!hasCanonical) errors.push('patch mode requires an existing valid unico-page.json');
    if (!Array.isArray(input.operations) || input.operations.length === 0) {
      errors.push('operations must be a non-empty array in patch mode');
    }
...
function validateDesignProfile(profile, mode, hasCanonical, errors, warnings) {
  ...
  if (profile === undefined) {
    if (isCompletePage) {
      warnings.push('No designProfile was recorded; apply the bundled Unico design guidance before final delivery');
    }
    return;
  }
  ...
  if (hasPath(profile, 'source') && !string(profile.source).trim()) {
    errors.push('designProfile.source must be a non-empty string');
  }
