...
Use this library to compose a page-specific visual language. Select a family from the brief, audience, imagery, and conversion goal; do not copy a complete template. Record the selected family and axes in `designProfile`. Compare multiple directions only when the brief is genuinely ambiguous.
